...
if [ -e "geog_complete.tar.bz2" ]; then
	mkdir -p "$geog_path"
	$fet || [ "$(ls -A $geog_path)" == "" ]	&& $unsudo tar xjvf geog_complete.tar.bz2 -C "$geog_path" || echo "Already extracted GEOG"
elif [ -e "geog_minimum.tar.bz2" ]; then
	mkdir -p "$geog_path"
	$fet || [ "$(ls -A $geog_path)" == "" ]	&& $unsudo tar xjvf geog_minimum.tar.bz2 -C "$geog_path" || echo "Already extracted GEOG"
fi
